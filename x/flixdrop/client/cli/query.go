...
Example:
$ %s query claim claimable-for-action omniflix1plrwrutl8565437tm2hl427hduwg8m8fes8z24 1
`,
				version.AppName,
			),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			action, err := strconv.ParseUint(args[1], 10, 64)
			if err != nil {
				return fmt.Errorf("invalid action number: %s", args[1])
			}
